...
exports.problem_details_get = function (req, res) {
  async.parallel(
    [
      (callback) => {
        Contest.findById(req.params.cid).exec(callback);
      },
      (callback) => {
        Problem.findById(req.params.pid).exec(callback);
      },
    ],
    function (err, result) {
      if (err) {
...
exports.contest_problem_update_get = function (req, res) {
  res.render("layout", {
    content: "problem/add-form",
    contestid: req.params.cid,
  });
};

exports.contest_problem_update_post = [
  upload.any(),
  function (req, res) {
    var problems_list = req.body.problems;
    console.log(problems_list, req.params);
    ...
    Contest.findByIdAndUpdate(req.params.cid, update, options, function (
      err,
      contest
    ) {
      if (err) {
